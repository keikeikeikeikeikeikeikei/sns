...
class BlogController
{
    ...
    public function show(Request $request, Response $response, array $args): Response
    {
        $GLOBALS['request_user_id'] = $request->getAttribute('user_id');
        $blog = Post::blog()
            ->with(['user', 'reactions', 'answers.user', 'answers.reactions', 'quotesAsSource.quotingPost.user', 'quotesAsQuoting.sourcePost.user'])
            ->find($args['id']);
    ...
    #[OA\Post(
        ...
        responses: [
            new OA\Response(response: 201, description: 'コメント投稿成功'),
            new OA\Response(response: 404, description: 'ブログが見つからない'),
        ]
    )]
    public function storeComment(Request $request, Response $response, array $args): Response
    {
        ...
        $comment = Post::create([
            ...
            'type' => Post::TYPE_BLOG,
            'content_long' => $data['content'],
            'parent_post_id' => $blogId,
        ]);

        $comment->load('user');

        return $this->jsonResponse($response, $this->formatComment($comment), 201);
    }
    ...
    private function formatBlog(Post $blog, bool $fullContent = false): array
    {
        ...
        $userReactions = [];
        if ($currentUserId && $blog->relationLoaded('reactions')) {
        ...
        $data = [
            ...
            'type' => $blog->type,
            'title' => $blog->title,
            'content' => $fullContent ? $blog->content_long : mb_substr($blog->content_long ?? '', 0, 100) . '...',
            'user' => [
                'id' => $blog->user->id,
        ...
        if ($fullContent && $blog->relationLoaded('answers')) {
            $data['comments'] = $blog->answers->map(fn($c) => $this->formatComment($c));
        }

        if ($fullContent && $blog->relationLoaded('quotesAsQuoting')) {
            $data['quoted_posts'] = $blog->quotesAsQuoting->map(fn($quote) => [
    ...
    private function formatComment(Post $comment): array
    {
        $currentUserId = $GLOBALS['request_user_id'] ?? null;
        $userReactions = [];
        if ($currentUserId && $comment->relationLoaded('reactions')) {
